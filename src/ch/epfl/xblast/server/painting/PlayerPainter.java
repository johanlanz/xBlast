...
public final class PlayerPainter {
        private static final int PLAYER_COEFFICIENT = 20;
        private static final int INVULNERABLE_COEFFICIENT = 80;
    ...
    public static byte byteForPlayer(int tick, Player player){
        ...
        if (player.isAlive()) {
            int byteImage = PLAYER_COEFFICIENT * player.id().ordinal();
            ...
            if (!lifeState.equals(State.DYING)) {
                ...
                if (lifeState.equals(State.INVULNERABLE) && tick % 2 == 1) {
                    byteImage = INVULNERABLE_COEFFICIENT;
                }
                
                //On ajoute 3 fois l'ordinal pour s'accorder aux images fournies. 
                ...
                if(dir.isHorizontal()){
                    remainder = player.position().x() % 4;
                }else{
                    remainder = player.position().y() % 4;
                }
